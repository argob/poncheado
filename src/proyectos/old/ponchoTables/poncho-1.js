...
//#####################################################################
  ...
  function ponchoTable(opt){
   ...
   /*jQuery.ajax({
      type: 'GET',
      url: 'https://spreadsheets.google.com/feeds/list/'+opt.idSpread+'/'+opt.hojaNumero+'/public/values?alt=json',
      beforeSend:function(){
        // this is where we append a loading image
        jQuery('#thisid').html('<div class="loading"><img src="loading.gif" alt="Loading..." /></div>');
      },
      success:function(data){
        // successful request; do something with the data
        jQuery('#thisid').html(data);
        console.log(data)
        data.feed.entry.forEach(function(e) {
          jQuery('#thisid').append('<div>'+e.gsx$descripcion.$t+'</div>');
        });
      },
      error:function(){
        // failed request; give feedback to user
        jQuery('#thisid').html('<p class="error"><strong>Oops!</strong> Try that again in a few moments.</p>');
      }
    });*/
